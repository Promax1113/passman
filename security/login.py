...
def login():
    ...
    if not os.path.exists(filepath_complete):
        ...
        create_saltfile(f"{os.path.expanduser(config["location"])}/{config["hashpath"]}/{config["saltfile"]}",
                        os.urandom(16))
        did_hashfile_exist = False

    login_details = {"username": "", "password": ""}
    if not did_hashfile_exist:
        print("Since this is your first time accessing this program you will need to set up a new login.")
