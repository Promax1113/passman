...
from .login import parse_hashfile
...
def gen_fernet_key(login: bytes) -> Fernet:
    ...
    salt = parse_hashfile(hashpath + saltfile)
    
    kdf = PBKDF2HMAC(
        algorithm=hashes.SHA256(),
        length=32, salt=salt.encode(),
        iterations=390000
        )
...
def read_password(filename: str, login: dict):
    ...
    conf = parse_config()
    with open(os.path.expanduser(conf["location"] + conf["hashpath"] + "/passwords/" + filename + ".enc"), "rb") as file:
        return f.decrypt(file.read()).decode()
    
def create_password(password: dict, login: dict):
    f = gen_fernet_key(json.dumps(login).encode())
    conf = parse_config()
    with open(os.path.expanduser(conf["location"] + conf["hashpath"] + "/passwords/" + password["name"] + ".enc"), "wb") as file:
